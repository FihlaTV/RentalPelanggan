...
public class TabStatus5 extends Fragment {
    ...
    public void getDataPemesanan() {
        try {
            ...
            mDatabase.child("pemesananKendaraan").child(status5).orderByChild("idPelanggan").equalTo(idPelanggan).addValueEventListener(new com.google.firebase.database.ValueEventListener() {
                @Override
                public void onDataChange(com.google.firebase.database.DataSnapshot dataSnapshot) {
                    if (dataSnapshot.exists()) {
                        for (com.google.firebase.database.DataSnapshot postSnapshot : dataSnapshot.getChildren()) {
                            ...
                            adapter = new TabStatus5Adapter(getActivity(), pemesananModel);
